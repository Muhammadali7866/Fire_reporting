...
from .forms import Loginform,fire_reports,ADD_team
from .models import add_team, fire_history,fire_report
...
def dashboard(request):
    ...
    totalnewequest = fire_report.objects.filter(status__isnull=True).count()
    totalAssign = fire_report.objects.filter(status='Assigned').count()
    totalontheway = fire_report.objects.filter(status='Team On the Way').count()
    totalworkprocess = fire_report.objects.filter(status='Fire Relief Work in Progress').count()
    totalreqcomplete = fire_report.objects.filter(status='Request Completed').count()
    totalfire = fire_report.objects.all().count()
...
def new_request(request):
    if not request.user.is_authenticated:
        return redirect('login')
    fm=fire_report.objects.filter(status__isnull=True)
...
def view_request(request,id):
    ...
    fm=fire_report.objects.get(id=id)
    print(fm)
    reportcount=fire_history.objects.filter(firereport=fm).count()
    report1 = fire_history.objects.filter(firereport=fm)
    ...
    try:
        ...
        if request.method=="POST":
            ...
            request_tracking=fire_history.objects.create(firereport=fm,status=status,remark=remark)
            fm.status=status
...
def delete_request(request,id):
    if not request.user.is_authenticated:
        return redirect('login')
    user=fire_report.objects.get(id=id)
    user.delete()
...
def view_status(request):
    ...
    fire=fire_report.objects.filter(Q(mobile=info) | Q(location=info) | Q(name=info))

    return render(request,"main/view_status.html",{"report":fire})

def viw_status_detail(request,pid):
    fm=fire_report.objects.get(id=pid)
    fm2=fire_history.objects.filter(firereport=fm)
    count=fire_history.objects.filter(firereport=fm).count()
